...
export async function SiteFooter() {
  ...
  return (
    <footer className="surface-dark mt-0 border-t border-ivory-100/10">
      <Container className="relative z-[1] py-16">
        <div className="grid gap-12 md:grid-cols-[1.4fr_1fr_1fr_1fr]">
          <div>
            ...
            <p className="mt-6 t-caption text-ivory-100/50">Every number carries an evidence label. Missing data is shown as RESEARCH REQUIRED — never invented. Planning outputs are conceptual and require professional validation. Nothing here is investment advice.</p>
          </div>
...
function FooterCol({ title, items }: { title: string; items: { label: string; href: string }[] }) {
  return (
    <div>
      <p className="t-overline text-leaf-300">{title}</p>
...
export function PrevNext({ prev, next, label }: { prev?: { name: string; href: string }; next?: { name: string; href: string }; label: string }) {
  ...
  return (
    <nav aria-label={`${label} navigation`} className="grid gap-4 sm:grid-cols-2">
      {prev ? <Link href={prev.href} className="group rounded-[var(--radius-control)] border hairline p-5 hover:border-coconut-800"><span className="t-overline text-neutral-500">← Previous {label}</span><span className="t-h4 mt-1 block group-hover:underline">{prev.name}</span></Link> : <span />}
      {next && <Link href={next.href} className="group rounded-[var(--radius-control)] border hairline p-5 text-right hover:border-coconut-800"><span className="t-overline text-neutral-500">Next {label} →</span><span className="t-h4 mt-1 block group-hover:underline">{next.name}</span></Link>}
    </nav>
  );
}
...
export function PageIntro({ overline, title, lede, breadcrumbs, children, dark = true }: { overline?: string; title: ReactNode; lede?: ReactNode; breadcrumbs?: { label: string; href?: string }[]; children?: ReactNode; dark?: boolean }) {
  return (
    <div className={cx(dark ? "surface-dark" : "surface-ivory", "pt-8 pb-14 md:pt-10 md:pb-20")}>
      <Container className="relative z-[1]">
        {breadcrumbs && <Breadcrumbs items={breadcrumbs} dark={dark} className="mb-8" />}
        {overline && <p className={cx("t-overline mb-4", dark ? "text-leaf-300" : "text-leaf-500")}>{overline}</p>}

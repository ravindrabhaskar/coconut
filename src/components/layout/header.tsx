...
function HeaderInner({ nav, pathname }: { nav: NavGroup[]; pathname: string }) {
  ...
  return (
    <header className={cx("sticky top-0 z-50 border-b transition-colors duration-300", dark ? "border-transparent bg-coconut-950/0 text-ivory-50" : "border-neutral-200/80 bg-ivory-50/90 text-neutral-900 backdrop-blur-md")}
      onMouseLeave={leave}>
          ...
          <ul className="flex items-center gap-1">
            {nav.map((g, i) => (
              <li key={g.label} onMouseEnter={() => enter(i)} onFocus={() => enter(i)} className={cx(g.secondary && "hidden xl:block")}>
                <Link href={g.href} className={cx("t-nav rounded-[var(--radius-control)] px-3.5 py-2 transition-colors", open === i ? (dark ? "bg-ivory-50/10" : "bg-neutral-100") : "hover:opacity-80")} aria-expanded={open === i} aria-haspopup="true">
                  {g.label}
                </Link>
              </li>
            ))}
            {nav.some((g) => g.secondary) && (
              <li onMouseEnter={() => enter(-1)} onFocus={() => enter(-1)} className="xl:hidden">
                <button className={cx("t-nav rounded-[var(--radius-control)] px-3.5 py-2 transition-colors", open === -1 ? (dark ? "bg-ivory-50/10" : "bg-neutral-100") : "hover:opacity-80")} aria-expanded={open === -1} aria-haspopup="true">More</button>
              </li>
            )}
          </ul>
        </nav>
      ...
      {/* Mega menu (desktop) */}
      {open !== null && active && (
        <div onMouseEnter={() => enter(open)} className="absolute inset-x-0 top-full hidden border-b border-neutral-200 bg-cocos text-neutral-900 shadow-[0_24px_60px_-30px_rgba(0,0,0,0.35)] lg:block">
          <div className="mx-auto grid max-w-[1440px] grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-10 px-12 py-10">
            {active.columns.filter((c) => c.items.length).map((col) => (
              <div key={col.heading}>
                ...
                <ul className="space-y-1">
                  {col.items.map((it) => (
                    <li key={it.href}>
                      <Link href={it.href} className="group block rounded-[var(--radius-control)] px-2 py-1.5 -mx-2 hover:bg-neutral-100">
            ...
            {active.featured && (
              <Link href={active.featured.href} className="surface-dark rounded-[var(--radius-media)] p-6 hover:brightness-110">
...
function SearchButton({ dark }: { dark: boolean }) {
  return (
    <Link href="/search" className={cx("t-nav hidden items-center gap-2 rounded-[var(--radius-control)] border px-3 py-1.5 md:inline-flex", dark ? "border-ivory-100/25 hover:bg-ivory-50/10" : "border-neutral-300 hover:bg-neutral-100")} aria-label="Search the platform">
...
function MobileSearch({ onDone }: { onDone: () => void }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (q.trim()) { router.push(`/search?q=${encodeURIComponent(q.trim())}`); onDone(); } }} className="mb-2 flex gap-2">
      ...
      <button className="tap rounded-[var(--radius-control)] bg-coconut-950 px-4 text-ivory-50 t-cta text-xs">Go</button>
    </form>
  );
}
...
export function Logo({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 32 32" fill="none" aria-hidden="true">
      ...
      <circle cx="16" cy="16" r="2" fill="var(--color-leaf-300)" />
    </svg>
  );
}

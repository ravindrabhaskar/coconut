      ...
      <Section surface="ivory">
        <Container>
          <ul className="grid gap-px overflow-hidden rounded-[var(--radius-media)] border hairline bg-neutral-200 sm:grid-cols-2 lg:grid-cols-3">
            {segments.map((s) => <li key={s.id} className="bg-cocos p-6 hover:bg-ivory-100"><Link href={`/customers/${s.slug}`} className="group block h-full"><Badge>{s.kind}</Badge><p className="t-h4 mt-3 group-hover:underline underline-offset-4">{s.name}</p><p className="t-caption mt-2">{s.summary}</p><p className="t-data mt-3 text-neutral-500">{s.productIds.map((id) => products.find((p) => p.id === id)?.name).filter(Boolean).slice(0, 4).join(" · ")}</p></Link></li>)}
          </ul>
        </Container>
      </Section>
    </>
